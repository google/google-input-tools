...
#elif defined(OS_CYGWIN) || defined(OS_NACL)
...
#if defined OS_LINUX || defined OS_CYGWIN || defined __ANDROID__
...
// using namespace std so we can compile google3 codes
...
#include "i18n/input/engine/stubs/nacl/newlib_port/byteswap.h"
#define bswap_16(x) i18n_input::engine::stubs::byteswap_16(x)
#define bswap_32(x) i18n_input::engine::stubs::byteswap_32(x)
#define bswap_64(x) i18n_input::engine::stubs::byteswap_64(x)
...
#if defined(COMPILER_MSVC) && !defined(va_copy)
...
// GCC-specific features
     ...
     defined(OS_MACOSX) || defined(__ANDROID__)) && !defined(SWIG)
...
inline void *aligned_malloc(size_t size, int minimum_alignment) {
  ...
  // give up
  return NULL;
#elif defined(OS_CYGWIN) || defined(__ANDROID__) || defined(OS_NACL)
...
#define strtof(a,b)   stof((a), reinterpret_cast<size_t*>(b))
...
#if defined(COMPILER_GCC3) || defined(COMPILER_ICC) || \
    defined(OS_MACOSX) || defined(COMPILER_MSVC)
#define __STD std
#endif
...
#define LANG_CXX11 1
// TODO(jyasskin): s/LANG_CXX0X/LANG_CXX11/g and remove it.
#define LANG_CXX0X 1
#endif
...
namespace std {}  // Avoid error if we didn't see std.
using namespace std;  // Just like VC++, we need a using here.
namespace __gnu_cxx {}  // Avoid error if we didn't see __gnu_cxx.
...
inline float log(int v) {
  return log(static_cast<float>(v));
}
...
// TODO(zkfan): Open these two macros in util/gtl/manual_constructor.h instead.
...
// TODO(jdennett): Eliminate this when we no longer need to support compilation
// with Crosstool V15.
#if defined(__clang__)
...
// Modified by: zyb@google.com (Yuanbo Zhang)
//
// These are weird things we need to do to get this compiling on
// random systems (and on SWIG).
