...
goog.provide('i18n.input.keyboard.Model');
...
goog.require('i18n.input.keyboard.LayoutEvent');
goog.require('i18n.input.keyboard.ParsedLayout');
 ...
 */
i18n.input.keyboard.Model = function() {
  goog.base(this);
   ...
   *
   * @type {Object.<i18n.input.keyboard.ParsedLayout|boolean>}
   * @private
   */
  this.layouts_ = {};
  ...
  // Note: don't move this to anywhere else, because it can remain
  // loose-coupleing with less dependencies, especially for unit tests.
  goog.exportSymbol('google.elements.keyboard.loadme',
...
/**
 * @define {string} The base url for the layout js files.
 */
i18n.input.keyboard.Model.STATIC_DIR =
    '//ssl.gstatic.com/inputtools/js/kbd/1/';
 ...
 */
i18n.input.keyboard.Model.prototype.loadLayout = function(layoutCode) {
  ...
  if (parsedLayout == undefined) {
    ...
    // For hangul, load hangul.js, which will then load ko.js.
    if (layoutCode == 'ko') {
      layoutCode = 'hangul';
    }
    i18n.input.keyboard.Model.loadLayoutScript_(layoutCode);
  } else if (parsedLayout) {
    this.dispatchEvent(new i18n.input.keyboard.LayoutEvent(
        i18n.input.keyboard.EventType.LAYOUT_LOADED, parsedLayout.view));
  }
};
 ...
 */
i18n.input.keyboard.Model.prototype.activateLayout = function(
    layoutCode) {
  if (!layoutCode) return;
  ...
  if (this.activeLayout_ != layoutCode) {
    ...
    if (parsedLayout) {
      ...
      this.clearHistory_();
      this.dispatchEvent(new i18n.input.keyboard.LayoutEvent(
          i18n.input.keyboard.EventType.LAYOUT_ACTIVATED, parsedLayout.view));
 ...
 */
i18n.input.keyboard.Model.prototype.getCurrentLayout = function() {
 ...
 */
i18n.input.keyboard.Model.prototype.translate = function(
  ...
  if (parsedLayout.isAmbiChars(history.ambi + chars)) {
    ...
    history.ambi += chars;
  } else if (parsedLayout.isAmbiChars(chars)) { // See b/7080140.
 ...
 */
i18n.input.keyboard.Model.prototype.hasTransforms = function() {
  var parsedLayout = this.layouts_[this.activeLayout_];
 ...
 *     input box. This will be used to compare with the history states.
 */
i18n.input.keyboard.Model.prototype.processBackspace = function(
    charsBeforeCaret) {
  this.matchHistory_(charsBeforeCaret);
 ...
 * @private
 */
i18n.input.keyboard.Model.prototype.onLayoutLoaded_ = function(layout) {
  if (layout.id == 'hangul') { // For Korean, load hangul.js then ko.js.
 ...
 * @private
 */
i18n.input.keyboard.Model.prototype.matchHistory_ = function(text) {
 ...
 * @private
 */
i18n.input.keyboard.Model.prototype.clearHistory_ = function() {
 ...
 * @private
 */
i18n.input.keyboard.Model.loadLayoutScript_ = function(layoutCode) {
